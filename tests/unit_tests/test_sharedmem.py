...
class TestSharedArrays:
    def test_single_array_creation(self):
        ...
        with managed_memory(sharedmem.SharedArrays([spec], create=True)) as shared_arrays:
    ...
    def test_multiple_array_creation(self):
        ...
        with managed_memory(sharedmem.SharedArrays(specs, create=True)) as shared_arrays:
...
class TestDoubleBufferedArrays:
    def test_reads_and_writes_occur_on_separate_buffers(self):
        specs = [('arr', (10,), np.uint8)]
        with managed_memory(sharedmem.DoubleBufferedArrays(specs, create=True)) as dbl_buffered:
